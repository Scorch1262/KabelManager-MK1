...
def _path_midpoint_normal(a, b):
    """Einheitsvektor senkrecht zur lokalen Segmentrichtung (rein
    kosmetisch fuer die Platzierung von Leitungs-/Kabel-Bezeichnungen
    seitlich neben der Linie, siehe pathMidpointNormal() in
    static/app.js)."""
...
def build_smooth_path_segments(points, dir1, dir2):
    """Python-Nachbildung von buildSmoothPath() in static/app.js - aus der
    urspruenglichen Netzwerkplan-Software uebernommene Leitungsfuehrung
    (weiche Bezier-Kurve, Anti-Verknoten-Verhalten an Ports/Pins: die
    Leitung tritt an einem konkreten Anschlusspunkt senkrecht zur
    jeweiligen Seite aus). MUSS mit der JS-Version mathematisch exakt
    uebereinstimmen, damit die PDF-Leitungen an derselben Stelle und mit
    derselben Form verlaufen wie die Webansicht.
...
def build_pdf(cfg, theme_name):
    ...
    # Bogen macht).
    # Bounding Box aller Elemente UND der tatsaechlichen Bezier-Kurven
    # (inkl. Kontrollpunkte, nicht nur Anfang/Ende!) ermitteln, um den Plan
    # passend auf die Seite (Querformat) zu skalieren. Eine kubische
    # Bezier-Kurve liegt immer innerhalb der konvexen Huelle ihrer 4
    # Kontrollpunkte - deren Einbeziehung verhindert daher zuverlaessig,
    # dass ausladende Kurven (z. B. wenn eine Leitung seitlich neben allen
    # Elementen einen Bogen macht) am Seitenrand abgeschnitten werden.
    xs, ys = [], []
    for el in elements:
        x = el.get("x", 0) or 0
        y = el.get("y", 0) or 0
    ...
    for conn in connections:
        ...
        waypoints = [(wp["x"], wp["y"]) for wp in (conn.get("waypoints") or [])
                     if isinstance(wp, dict) and "x" in wp and "y" in wp]
        for seg in build_smooth_path_segments([p1] + waypoints + [p2], dir1, dir2):
            for pt in seg[1:]:
                xs.append(pt[0])
                ys.append(pt[1])
    ...
    # (siehe build_smooth_path_segments()/buildSmoothPath in static/app.js):
    # weiche Bezier-Kurve mit senkrechtem Austritt an Ports/Pins.
    c.setLineJoin(1)
    c.setLineCap(1)
